...
static
int wav_read_frames(pcm_reader_t *preader, void *buffer, size_t nframes)
{
    int rc;
    unsigned nbytes;
...
int wav_fmt(wav_reader_t *reader, uint32_t size)
{
    ...
    reader->sample_format.bits_per_channel = (uint8_t)wValidBitsPerSample;
    reader->sample_format.bytes_per_frame = (uint8_t)nBlockAlign;
    reader->sample_format.channels_per_frame = (uint8_t)nChannels;
    ...
    *data_length = 0;
    ...
    while ((fcc = riff_next_chunk(reader, &chunk_size)) != 0) {
        if (fcc == RIFF_FOURCC('f','m','t',' ')) {
            ...
            reader->data_offset = (int32_t)pcm_tell(&reader->io);
            break;
        } else {

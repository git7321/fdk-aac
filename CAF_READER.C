...
int caf_desc(caf_reader_t *reader, int64_t chunk_size)
{
    ...
    desc->sample_rate        = (uint32_t)mSampleRate;
...
int caf_info(caf_reader_t *reader, int64_t chunk_size)
{
    ...
    if (chunk_size < 4 || (buf = malloc((size_t)(chunk_size+1))) == 0)
        return -1;
    n = pcm_read(&reader->io, buf, (uint32_t)chunk_size);
    if (n != chunk_size)
        return -1;
    buf[n] = 0;
    key = buf + 4;
...
static
int caf_read_frames(pcm_reader_t *preader, void *buffer, size_t nframes)
{
    int rc;
    unsigned i, j, nbytes;
    ...
    if (nframes > reader->length - reader->position)
        nframes = (size_t)(reader->length - reader->position);
    nbytes = nframes * bpf;
    if (nbytes) {
    ...
    *data_length = 0;
    ...
    while ((fcc = caf_next_chunk(reader, &chunk_size)) != 0) {
        if (fcc == M4AF_FOURCC('d','e','s','c')) {
            ...
            if (apple_chan_chunk(&reader->io, (uint32_t)chunk_size,
                                 &reader->sample_format, reader->chanmap) < 0)
                goto FAIL;
        } else if (fcc == M4AF_FOURCC('d','a','t','a')) {
            TRY_IO(pcm_skip(&reader->io, 4));
            *data_length = (chunk_size == ~0UI64) ? chunk_size : chunk_size - 4;
